...
--------------------------------------------------------- */
	...
	function rating_bar($page_id, $units='', $static='') {
		...
		require_once ('static/connect.inc.php'); // get the db connection info
		...
		if ($count == 0) {
			$req = "INSERT INTO `ratings` (`vote_id` ,`page_id` ,`total_votes` ,`total_value` ,`used_ips` ) VALUES (NULL , '".$page_id."', '0', '0', NULL);";
		...
		$rating_width = $rating1 = $rating2 = 0;
		if($count!=0){
		...
		if ($static == 'static') {
				...
				$static_rater[] .= '</ul>';
				$static_rater[] .= '<p class="static"><strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' )</p>';
				$static_rater[] .= '</div>';
				$static_rater[] .= '</div>'."\n\n";

				return join("\n", $static_rater);
		} 
		else if ($static == 'perso') {
			$static_rater = array();
			$static_rater[] .= "\n".'<div class="ratingblock">';
			$static_rater[] .= '<div id="unit_long'.$page_id.'">';
			$static_rater[] .= '<ul id="unit_ul'.$page_id.'" class="unit-rating" style="width:'.$rating_unitwidth*$units.'px;">';
			...
			for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
			   if(!$voted) { // if the user hasn't yet voted, draw the voting stars
				  $rater.='<li><a href="static/vote/db.php?j='.$ncount.'&amp;q='.$page_id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$ncount.' sur '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
			   }
			}
