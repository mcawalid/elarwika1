...
if (!isset($_SESSION)) {
  session_start();
	...
	$sql = "SELECT count(*) FROM commande WHERE id_user=".$_SESSION['id_vente'];
	...
	$req = "SELECT * from commande WHERE id_user=".$_SESSION['id_vente']." order by id DESC LIMIT ".intval($_GET['debut']).', '.$nb_affichage_par_page;
	mysqli_query($base,"SET NAMES UTF8");
											...
											<table class="table table-bordered table-hover table-striped tablesorter">
												...
												<?php while($resultat=mysqli_fetch_array($selectcle)){?>
													<tr>
														...
														<td style="vertical-align:middle;"><?php echo getDateFormat($resultat['date']).' à '.str_pad($resultat['heure'], 2, "0", STR_PAD_LEFT).'h'.str_pad($resultat['minute'], 2, "0", STR_PAD_LEFT) ?>
														<td style="vertical-align:middle;">
															<?php
																$req= "SELECT sum(c.prix*c.quantite) as count FROM commande_detail c where c.id_commande=".$resultat['id'];
																mysqli_query($base,"SET NAMES UTF8");
																$count = mysqli_fetch_array(mysqli_query($base, $req));
																echo $count['count']." DA";
															?>
														</td>
														<td style="vertical-align:middle;text-align:center"><?php echo $resultat['date_insert']?></td>
													...
													<td style="text-align:center">
														<a href="espace_commande_detail.php?id=<?php echo $resultat['id']?>">
															<img src="images/voir.png" title="Consulter le detail de cette commande">
														</a>
													</td>
												</tr>
											<?php } ?>
